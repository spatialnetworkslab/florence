import {
  transitionLinearRing, implodeLinearRing, explodeLinearRing
} from '../transitionLogic/transitionLinearRing.js'
...
export default function Polygon (fromPolygon, toPolygon) {
  ...
  let outerRingInterpolator = transitionLinearRing(fromOuterRing, toOuterRing)
  ...
  if (holes > 0) {
    ...
    for (let i = 1; i <= holes; i++) {
      ...
      if (fromHole && toHole) {
        interpolatorArray.push(transitionLinearRing(fromHole, toHole))
      }

      if (fromHole && !toHole) {
        interpolatorArray.push(implodeLinearRing(fromHole))
      }

      if (!fromHole && toHole) {
        interpolatorArray.push(explodeLinearRing(toHole))
      }
    }
